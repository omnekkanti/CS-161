...
public class ScannerAaron {
    ...
    public String next() {
        ...
        if (pattern.matcher("").matches()) {//checks if delimiter is empty string
            ...
            while (cursorPosition + count <= source.length() && pattern.matcher(source.substring(cursorPosition, cursorPosition + count)).matches()) {
                if (VERBOSE) {
                    System.out.printf("'%s' is still a delimiter. Skip!%n", source.substring(cursorPosition, cursorPosition + count));
                }
                count++;
            }
            ...
            if (cursorPosition + count <= source.length()) {
                if (VERBOSE) {
                    System.out.printf("'%s' is no longer a delimiter. STOP!%n", source.substring(cursorPosition, cursorPosition + count));
                }
            } else {
                if (VERBOSE) {
        ...
        while (cursorPosition + count <= source.length() && !pattern.matcher(source.substring(cursorPosition, cursorPosition + count)).find()) {
            if (VERBOSE) {
                System.out.printf("'%s' is still a token. Skip!%n", source.substring(cursorPosition, cursorPosition + count));
            }
            count++;
        }
        ...
        if (cursorPosition + count <= source.length()) {
            if (VERBOSE) {
                System.out.printf("'%s' is no longer a token. STOP!%n", source.substring(cursorPosition, cursorPosition + count));
            }
        } else {
            if (VERBOSE) {
                System.out.println("Reached end of string.");
        ...
        if (VERBOSE) {
            System.out.printf("Our token is: '%s'! yay!%n", token);
        }
        ...
        if (VERBOSE) {
            System.out.printf("Exit method. Cursor now at: %d%n", cursorPosition);
        }
        return token;
    }
    ...
    public String nextLine() {
        ...
        while (cursorPosition + count < source.length() && !currentChar.equals("\n")) {
            ...
            count++;
            if (VERBOSE) {
                System.out.printf("String so far: '%s'%n", out.toString());
            }
        }
